...
export default {
    data() {
        ...
        created() {
        let apiURL = `http://127.0.0.1:8000/api/clients-edit/${this.$route.params.id}`;
        axios.get(apiURL).then((res) => {
        this.clientUpdate = res.data;
    });
    },

    methods: {
        updateClient() {
        let apiURL = `http://127.0.0.1:8000/api/clients-update/${this.$route.params.id}`;
        if (window.confirm("¿Realmente desea modificar el cliente?")) {
            axios
            .put(apiURL, this.clientUpdate)
            .then((res) => {
            console.log(res);
